...
class Matrix {
  ...
  public equals(other: Matrix): boolean {
    return this._matrix.every((row, i) =>
      row.every((num, j) => num === other.getElementAt(i, j)),
    );
  }

  public multiply(other: Matrix): Matrix {
    const result = new Matrix(this.rows.length, other.columns.length);
    result._matrix = result._matrix.map((resultRow, rowIndex) =>
      resultRow.map((_, columnIndex) =>
        this.getRow(rowIndex).reduce(
          (sum, value, i) => sum + value * other.getElementAt(i, columnIndex),
          0,
        ),
      ),
    );
  ...
  public multiplyTuple(tuple: Tuple): Tuple {
    return Tuple.fromArray(
      this.multiply(Matrix.fromArray([tuple.toArray()])).matrix[0],
    );
  }
}
