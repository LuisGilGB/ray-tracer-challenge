...
class Canvas {
  ...
  writePixel(x: number, y: number, color: Color) {
    this.pixels[Math.round(y * this.width + x)] = color;
  }

  forEachPixel(callback: (x: number, y: number, color: Color) => void) {
  ...
  toPPM(): string {
    ...
    const capNumber = (min: number, max: number) => (value: number) =>
      Math.min(Math.max(value, min), max);
    const scaleColor = (value: number) =>
      Math.round(capNumber(0, 1)(value) * MAX_COLOR_VALUE);
    for (let y = 0; y < this.height; y++) {
      let line = '';
      for (let x = 0; x < this.width; x++) {
        const pixel = this.getPixel(x, y);
        const colors = [pixel.red, pixel.green, pixel.blue].map(value =>
          scaleColor(value).toString(),
        );
